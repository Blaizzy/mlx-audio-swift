...
| 7 | Ship | 20-21 | Integration tests and cleanup |
...
### Task 21: Clean up and documentation [K1]

**Tier**: 7 — Ship

**What**: Ensure all changes follow mlx-audio-swift coding conventions, remove any
VoxAlta-specific code, add inline documentation for public API methods, and verify
package documentation reflects the fork's extended feature set.
...
- Package README documents Qwen3-TTS capabilities (Base, VoiceDesign, CustomVoice, ICL)
...
This execution plan represents work on the **independent intrusive-memory fork** of mlx-audio-swift. We've chosen to maintain an extended feature set with full Qwen3-TTS capabilities (Base, VoiceDesign, CustomVoice, and ICL voice cloning) while the upstream project focuses on VoiceDesign-only support. Both approaches are valid; we've opted for feature completeness to support advanced audio generation workflows.
