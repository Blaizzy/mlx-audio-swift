...
extension SmokeTests.VADTests {

    @Suite("Sortformer Tests", .serialized)
    struct SortformerTests {
        ...
        private static func saveSegmentsJSON(
            ...
            print("\u{001B}[32mSaved results to \(url.path)\u{001B}[0m")
        }
        ...
        @Test func sortformerOfflineInference() async throws {
            ...
            let outputURL = temporaryTestOutputURL("sortformer_offline_results.json")
        ...
        @Test func sortformerStreamingInference() async throws {
            ...
            let outputURL = temporaryTestOutputURL("sortformer_streaming_results.json")
            defer { removeTemporaryTestOutput(outputURL) }
            try Self.saveSegmentsJSON(
