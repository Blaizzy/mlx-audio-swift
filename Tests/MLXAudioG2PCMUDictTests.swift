...
struct CMUDictIntegrationTests {
    ...
    @Test func pipelineHandlesDigraphs() throws {
        ...
        let knight = try pipeline.convert("knight")
        let knightIPA = knight.phonemes.units.map { $0.symbol }
        #expect(knightIPA.first == "n")

        let through = try pipeline.convert("through")
        #expect(through.phonemes.render().contains("θ"))
    }
